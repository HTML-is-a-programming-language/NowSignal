...
| --- | --- | --- | --- |
| `kma-data-go-kr-vilage-fcst` | KMA 초단기실황·초단기예보·단기예보 | `approved_for_dev` | 정상 Endpoint 3개 C-01 `pass`; 계획 오류 Case `not_run`, Run 10 C-11 `not_evaluated` |
...
| `C-11` | Source·Terms Binding은 `verified`. Run 10 예보 2개는 Source·시각·Grid·단위·License·Attribution 관련 공개 값을 Capture했지만 엄격한 Assertion을 평가하지 않아 `not_evaluated`; 실황은 `not_run`, C-11 `pass` 없음 | 정적 License는 공공누리 제1유형·`approved_for_dev`; 원문 우선·정정 연결의 실제 결합은 `not_run` | AirKorea 공공누리 제3유형·운영 `conditional_for_production`; 실제 응답 결합은 `not_run` | 같은 `LIC-AIR-001`; Run 9는 License·Attribution 결합을 평가하지 않음 | [C-11 사전등록 명세](./provider-c11-envelope-evidence-spec.md)는 `offline_spec_ready`. ProviderEnvelope에서 Source·시각·지역·단위·License·Attribution을 모두 추출·보존하고 하나라도 누락되면 fail. Capture 완료 Boolean이나 정적 등록부를 실제 결합 pass로 대체하지 않음 | KMA 예보의 남은 엄격한 Assertion과 다른 Provider의 Runtime Binding은 미완료. 실제 응답은 `owner_network_decision_required`; UI·구현은 `gate_3_4_approval_required` |
| `C-12` | Run 3과 Run 10 호출 전후 Repository·환경변수 원문·encoded 흔적·Scan 오류 0, `pass` | Run 5 호출 후 공통 검사 0, `pass` | Run 5 호출 후 공통 검사 0, `pass` | Run 9 사전·사후 추적·미추적·환경 원문·encoded와 Scan 오류 모두 0, Security conformance `pass` | 각 새 실행에서도 사전·사후 key 흔적과 Scan 오류가 0이고 Secret·Full URL·원 응답·예외 원문을 저장하지 않아야 함. 과거 pass가 미래 Run을 면제하지 않음 | 로컬 사전검사는 실행 Plan의 필수 조건. 새 외부 실행 자체는 `owner_network_decision_required` |
...
- KMA 특보 통보문·현황, AirKorea 근접 측정소·대기질 예보 등 미실행 C-01 Endpoint와 C-02~C-11 필수 Case
