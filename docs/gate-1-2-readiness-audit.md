...
- 제외 범위: 제품 코드, Google Forms 추가 조작, 참여자 모집·연락, API 활용신청·키 입력·실호출, 기관 문의, Production 승인
...
| --- | --- | --- | --- | --- |
| Gate 1 문제 검증 | `in_progress` | `ready_to_request_manual_preflight` | Interview 0건, 연구 결과 `not_verified` | 계획·Protocol·단계별 수동 Checklist는 준비됐다. 실제 문제 증거가 없으므로 통과 불가 |
| Gate 2 Public Data Feasibility | `in_progress` | `ready_for_manual_prerequisites` | Contract·오류·14일 Canary 모두 `not_run` | Phase 1 범위·계약·Evidence·호출 예산·Fail-closed 초안은 준비됐다. 실호출·권리 조건 확인 전 통과 불가 |
| Gate 3 Product Design | `blocked` | 해당 없음 | 없음 | Gate 1 핵심 문제 증거와 Gate 2 필수 Provider 가능성 확인 전 진입 금지 |

“비코드 준비 완료”는 Gate 통과나 제품 가능성 검증을 뜻하지 않는다. 지금 사용자에게 요구할 필수 수동 작업은 없고, [수동 작업 체크리스트](./manual-action-checklist.md)의 명시적 Trigger가 오기 전에는 현재 상태를 유지한다.
...
| Gate decision | `in_progress`, `blocked`, 향후 `passed` 또는 `rejected` | Gate 전체 판단 |
...
| A-02 | 모집 Email 공개, Form A 전달, 첫 Interview 전 차단 작업의 시점이 섞임 | Gate 1 수동 작업을 3단계 Trigger로 분리하고 단계별 완료 증거 명시 |
| A-03 | Form A 실제 문항의 가능한 시간대·삭제/철회 확인이 Protocol 최소수집 목록에 빠짐 | Screening 목록·수집 Inventory·동의 안내와 Build sheet를 동일하게 맞춤 |
| A-04 | 필수 동의 거부·부적격·적격 미참여 응답의 삭제 기한 표현이 섞임 | 동의 거부는 즉시, 부적격은 판정일 +7일, 적격 미선정·미참여는 확정일/모집 종료 중 이른 날 +7일, Interview 참여는 기존 `min(+90일, +30일)`로 통일 |
...
| A-08 | KMA 10,000회/일과 AirKorea 500회/일이 확정 quota처럼 보이면서 다른 곳에서는 충돌·공유 범위 미확인으로 기록 | 승인 quota는 `not_verified`로 유지하고 더 낮은 보수적 실검증 상한과 Canary 예산을 별도로 표시 |
...
사용자가 실제 문제 증거 수집을 명시적으로 승인할 때만 다음 순서로 재개한다.

1. 모집 안내·전용 Email 공개 전 법적 고지, Email 시험, 계정 보호와 운영값 확인
2. Form A Link 전달 전 최종 설정·외부 연동·분기·삭제일 Dry-run
3. 첫 Interview 전 Form B 생성·접근 제한·제출/삭제 Dry-run

### Gate 2

사용자가 API 실호출을 명시적으로 승인할 때만 활용신청과 Secret 입력을 요청한다. 키 값은 Chat·문서·Screenshot·Git에 남기지 않는다. Contract smoke와 Canary가 실제 Evidence를 만들기 전에는 상태를 바꾸지 않는다.
