...
| C-11 ProviderEnvelope Evidence 명세 | `done` | [C-11 명세](./provider-c11-envelope-evidence-spec.md) `offline_spec_ready`; Runtime Source·Terms·Null Binding `not_verified`, 실제 C-11 `not_run`, Network 0 | 실제 응답 검증 전 Endpoint별 Binding·Plan·Hash와 사용자 Network 결정 | Sanitized Manifest·필수 Assertion·Attribution Token만 공개 기록 |
