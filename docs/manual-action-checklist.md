...
| Contract smoke | `blocked` | KMA 정상 예보 Endpoint 3개, KMA 특보 목록과 AirKorea 측정소 목록 C-01 `pass`. Run 10 예보 2회는 C-01 전용 bounded pass, C-11 `not_evaluated`, C-12·Plan·Security `pass`, 누적 12회. AirKorea Run 9 원본 C-01 `fail`과 공식 Schema 충돌은 보존 | 남은 정상·오류 Contract, 엄격한 C-11, AirKorea 답변 분기와 Canary를 각각 새 Plan·Hash·사용자 결정으로 고정 | Sanitized Manifest·Canonical Plan Hash·C-12·공식 문서 대조 결과만 기록 |
...
| C-11 ProviderEnvelope Evidence 명세 | `done` | [C-11 명세](./provider-c11-envelope-evidence-spec.md) `offline_spec_ready`; KMA 예보 Source·Terms `verified`, Run 10 예보 2개 Capture-only `not_evaluated`, 나머지 실제 C-11 `not_run`; C-11 `pass` 없음 | 엄격한 Null·단위·License scope·Attribution·Layer Assertion과 사용자 Network 결정 | Sanitized Manifest·필수 Assertion·Attribution Token만 공개 기록 |
